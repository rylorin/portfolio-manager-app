...
class TradeUnit
{
  ...
  public const IRON_CONDOR = 18;
  public const SHORT_IRON_CONDOR = 19;
  ...
  public const stategyLabelMapping = [
    ...
    TradeUnit::IRON_CONDOR => 'iron condor',
    TradeUnit::SHORT_IRON_CONDOR => 'short iron condor',
  ];
  ...
  public const stategyMenuMapping = [
    ...
    'iron condor' => TradeUnit::IRON_CONDOR,
    'short iron condor' => TradeUnit::SHORT_IRON_CONDOR
  ];
  ...
  private function updateTradeUnit(): self
  {
    ...
    foreach ($this->openingTrades as $key => $statement) {
      ...
      if ((!$openingDate) || ($statement->getDate() < $openingDate))
        $openingDate = $statement->getDate();
      if ($statement->getDate() > $closingDate)
        $closingDate = $statement->getDate();
      // short put strategy to the wheel if we also have stocks
      if (($this->strategy == TradeUnit::SHORT_PUT) && ($statement->getStatementType() == Statement::TYPE_TRADE))
        $this->strategy = TradeUnit::THE_WHEEL;
